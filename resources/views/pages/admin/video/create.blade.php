                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="live-preview">
                            <form id="countryForm" method="post" action="{{route('video_store')}}" enctype="multipart/form-data">
                            @csrf
                            <div class="row gy-4">
  ...
  .onSuccess(async (event) => {
    ...
    const errorToast = (message) =>{
      ...
      try {
        ...
        const response = await axios.post('{{route('video_store')}}', formData)
        ...
        if(error?.response?.data?.form_error?.video){
            errorToast(error?.response?.data?.form_error?.video[0])
        }
      } finally{
            submitBtn.innerHTML =  `
                Submit
